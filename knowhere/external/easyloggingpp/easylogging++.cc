...
// Date/time
static const char* kDays[7] = {"Sundayaaa", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};
...
static void
abort(int status, const std::string& reason) {
...
const char*
LevelHelper::convertToString(Level level) {
...
Level
LevelHelper::convertFromString(const char* levelStr) {
...
void
LevelHelper::forEachLevel(base::type::EnumType* startIndex, const std::function<bool(void)>& fn) {
...
void
Configuration::log(el::base::type::ostream_t& os) const {
...
bool
Configuration::Predicate::operator()(const Configuration* conf) const {
...
bool
Configurations::parseFromFile(const std::string& configurationFile, Configurations* base) {
...
bool
Configurations::parseFromText(const std::string& configurationsString, Configurations* base) {
...
void
Configurations::setFromBase(Configurations* base) {
...
bool
Configurations::hasConfiguration(ConfigurationType configurationType) {
...
void
Configurations::set(Configuration* conf) {
    if (conf == nullptr) {
        return;
    }
...
void
Configurations::setToDefault(void) {
...
void
Configurations::setRemainingToDefault(void) {
    base::threading::ScopedLock scopedLock(lock());
#if defined(ELPP_NO_LOG_TO_FILE)
...
bool
Configurations::Parser::parseFromFile(const std::string& configurationFile, Configurations* sender,
                                      Configurations* base) {
...
bool
Configurations::Parser::parseFromText(const std::string& configurationsString, Configurations* sender,
                                      Configurations* base) {
    sender->setFromBase(base);
    bool parsedSuccessfully = false;
...
void
Configurations::Parser::ignoreComments(std::string* line) {
...
bool
Configurations::Parser::parseLine(std::string* line, std::string* currConfigStr, std::string* currLevelStr,
...
void
Configurations::unsafeSet(Level level, ConfigurationType configurationType, const std::string& value) {
    Configuration* conf = RegistryWithPred<Configuration, Configuration::Predicate>::get(level, configurationType);
    if (conf == nullptr) {
...
void
Configurations::setGlobally(ConfigurationType configurationType, const std::string& value, bool includeGlobalLevel) {
...
void
Configurations::unsafeSetGlobally(ConfigurationType configurationType, const std::string& value,
...
void
LogBuilder::convertToColoredOutput(base::type::string_t* logLine, Level level) {
...
void
Logger::configure(const Configurations& configurations) {
...
void
Logger::flush(Level level, base::type::fstream_t* fs) {
...
void
Logger::initUnflushedCount(void) {
...
void
Logger::resolveLoggerFormatSpec(void) const {
    base::type::EnumType lIndex = LevelHelper::kMinValid;
    LevelHelper::forEachLevel(&lIndex, [&](void) -> bool {
...
base::type::fstream_t*
File::newFileStream(const std::string& filename) {
...
std::size_t
File::getSizeOfFile(base::type::fstream_t* fs) {
    if (fs == nullptr) {
        return 0;
    }
...
bool
File::pathExists(const char* path, bool considerFile) {
    if (path == nullptr) {
        return false;
    }
#if ELPP_OS_UNIX
...
bool
File::createPath(const std::string& path) {
    if (path.empty()) {
        return false;
    }
...
std::string
File::extractPathFromFilename(const std::string& fullPath, const char* separator) {
...
void
File::buildStrippedFilename(const char* filename, char buff[], std::size_t limit) {
...
void
File::buildBaseFilename(const std::string& fullPath, char buff[], std::size_t limit, const char* separator) {
...
bool
Str::wildCardMatch(const char* str, const char* pattern) {
    while (*pattern) {
        switch (*pattern) {
            case '?':
                if (!*str)
...
std::string&
Str::ltrim(std::string& str) {
    str.erase(str.begin(), std::find_if(str.begin(), str.end(), [](char c) { return !std::isspace(c); }));
    return str;
}

std::string&
Str::rtrim(std::string& str) {
    str.erase(std::find_if(str.rbegin(), str.rend(), [](char c) { return !std::isspace(c); }).base(), str.end());
    return str;
}

std::string&
Str::trim(std::string& str) {
    return ltrim(rtrim(str));
}

bool
Str::startsWith(const std::string& str, const std::string& start) {
    return (str.length() >= start.length()) && (str.compare(0, start.length(), start) == 0);
}

bool
Str::endsWith(const std::string& str, const std::string& end) {
    return (str.length() >= end.length()) && (str.compare(str.length() - end.length(), end.length(), end) == 0);
}

std::string&
Str::replaceAll(std::string& str, char replaceWhat, char replaceWith) {
    std::replace(str.begin(), str.end(), replaceWhat, replaceWith);
    return str;
}

std::string&
Str::replaceAll(std::string& str, const std::string& replaceWhat, const std::string& replaceWith) {
...
void
Str::replaceFirstWithEscape(base::type::string_t& str, const base::type::string_t& replaceWhat,
...
bool
Str::cStringCaseEq(const char* s1, const char* s2) {
    if (s1 == nullptr && s2 == nullptr)
        return true;
    if (s1 == nullptr || s2 == nullptr)
        return false;
...
bool
Str::contains(const char* str, char c) {
    for (; *str; ++str) {
        if (*str == c)
            return true;
    }
...
char*
Str::convertAndAddToBuff(std::size_t n, int len, char* buf, const char* bufLim, bool zeroPadded) {
    char localBuff[10] = "";
...
char*
Str::addToBuff(const char* str, char* buf, const char* bufLim) {
    while ((buf < bufLim) && ((*buf = *str++) != '\0')) ++buf;
    return buf;
}

char*
Str::clearBuff(char buff[], std::size_t lim) {
    STRCPY(buff, "", lim);
...
char*
Str::wcharPtrToCharPtr(const wchar_t* line) {
...
const char*
OS::getWindowsEnvironmentVariable(const char* varname) {
...
#if ELPP_OS_ANDROID
std::string
OS::getProperty(const char* prop) {
...
const std::string
OS::getBashOutput(const char* command) {
...
std::string
OS::getEnvironmentVariable(const char* variableName, const char* defaultVal, const char* alternativeBashCommand) {
#if ELPP_OS_UNIX
...
void
DateTime::gettimeofday(struct timeval* tv) {
#if ELPP_OS_WINDOWS
    if (tv != nullptr) {
...
std::string
DateTime::getDateTime(const char* format, const base::SubsecondPrecision* ssPrec) {
...
std::string
DateTime::timevalToString(struct timeval tval, const char* format, const el::base::SubsecondPrecision* ssPrec) {
    struct ::tm timeInfo;
...
base::type::string_t
DateTime::formatTime(unsigned long long time, base::TimestampUnit timestampUnit) {
...
struct ::tm*
DateTime::buildTimeInfo(struct timeval* currTime, struct ::tm* timeInfo) {
#if ELPP_OS_UNIX
...
char*
DateTime::parseFormat(char* buf, std::size_t bufSz, const char* format, const struct tm* tInfo, std::size_t msec,
...
void
CommandLineArgs::setArgs(int argc, char** argv) {
    m_params.clear();
...
bool
CommandLineArgs::hasParamWithValue(const char* paramKey) const {
    return m_paramsWithValue.find(std::string(paramKey)) != m_paramsWithValue.end();
}

const char*
CommandLineArgs::getParamValue(const char* paramKey) const {
...
void
SubsecondPrecision::init(int width) {
    if (width < 1 || width > 6) {
...
void
LogFormat::parseFromFormat(const base::type::string_t& userFormat) {
...
void
LogFormat::updateDateFormat(std::size_t index, base::type::string_t& currFormat) {
...
void
LogFormat::updateFormatSpec(void) {
    // Do not use switch over strongly typed enums because Intel C++ compilers dont support them yet.
...
unsigned long
TypedConfigurations::getULong(std::string confVal) {
    bool valid = true;
...
std::string
TypedConfigurations::resolveFilename(const std::string& filename) {
...
void
TypedConfigurations::insertFile(Level level, const std::string& fullFilename) {
...
bool
TypedConfigurations::unsafeValidateFileRolling(Level level, const PreRollOutCallback& preRollOutCallback) {
    ...
    if (maxLogFileSize != 0 && currFileSize >= maxLogFileSize) {
        ...
        ELPP_INTERNAL_INFO(1, "Truncating log file [" << fname << "] as a result of configurations for level ["
                                                      << LevelHelper::convertToString(level) << "]");
        fs->close();
        preRollOutCallback(fname.c_str(), currFileSize, level);
...
bool
RegisteredHitCounters::validateEveryN(const char* filename, base::type::LineNumber lineNumber, std::size_t n) {
    base::threading::ScopedLock scopedLock(lock());
...
bool
RegisteredHitCounters::validateAfterN(const char* filename, base::type::LineNumber lineNumber, std::size_t n) {
    base::threading::ScopedLock scopedLock(lock());
    base::HitCounter* counter = get(filename, lineNumber);
    if (counter == nullptr) {
        registerNew(counter = new base::HitCounter(filename, lineNumber));
...
/// @return True if validation resulted in triggering hit. Meaning logs should be written everytime true is returned
bool
RegisteredHitCounters::validateNTimes(const char* filename, base::type::LineNumber lineNumber, std::size_t n) {
    base::threading::ScopedLock scopedLock(lock());
    base::HitCounter* counter = get(filename, lineNumber);
    if (counter == nullptr) {
        registerNew(counter = new base::HitCounter(filename, lineNumber));
...
bool
RegisteredLoggers::remove(const std::string& id) {
...
void
RegisteredLoggers::unsafeFlushAll(void) {
...
void
VRegistry::setLevel(base::type::VerboseLevel level) {
    base::threading::ScopedLock scopedLock(lock());
    if (level > 9)
...
void
VRegistry::setModules(const char* modules) {
    base::threading::ScopedLock scopedLock(lock());
...
bool
VRegistry::allowed(base::type::VerboseLevel vlevel, const char* file) {
    base::threading::ScopedLock scopedLock(lock());
...
void
VRegistry::setFromArgs(const base::utils::CommandLineArgs* commandLineArgs) {
...
bool
Storage::hasCustomFormatSpecifier(const char* formatSpecifier) {
...
void
Storage::installCustomFormatSpecifier(const CustomFormatSpecifier& customFormatSpecifier) {
...
bool
Storage::uninstallCustomFormatSpecifier(const char* formatSpecifier) {
    base::threading::ScopedLock scopedLock(customFormatSpecifiersLock());
...
void
Storage::setApplicationArguments(int argc, char** argv) {
...
void
DefaultLogDispatchCallback::handle(const LogDispatchData* data) {
#if defined(ELPP_THREAD_SAFE)
...
void
DefaultLogDispatchCallback::dispatch(base::type::string_t&& logLine) {
...
void
AsyncLogDispatchCallback::handle(const LogDispatchData* data) {
...
bool
AsyncDispatchWorker::clean(void) {
    std::mutex m;
...
void
AsyncDispatchWorker::emptyQueue(void) {
...
void
AsyncDispatchWorker::start(void) {
...
void
AsyncDispatchWorker::handle(AsyncLogItem* logItem) {
...
base::type::string_t
DefaultLogBuilder::build(const LogMessage* logMessage, bool appendNewLine) const {
...
void
MessageBuilder::initialize(Logger* logger) {
    m_logger = logger;
...
Writer&
Writer::construct(Logger* logger, bool needLock) {
    m_logger = logger;
...
void
PerformanceTracker::checkpoint(const std::string& id, const char* file, base::type::LineNumber line, const char* func) {
#if !defined(ELPP_DISABLE_PERFORMANCE_TRACKING) && ELPP_LOGGING_ENABLED
    if (m_enabled) {
        base::threading::ScopedLock scopedLock(lock());
        base::utils::DateTime::gettimeofday(&m_endTime);
...
std::ostream&
operator<<(std::ostream& ss, const StackTrace::StackTraceEntry& si) {
...
static void
logCrashReason(int sig, bool stackTraceIfAvailable, Level level, const char* logger) {
...
void
CrashHandler::setHandler(const Handler& cHandler) {
    m_handler = cHandler;
...
void
Helpers::crashAbort(int sig, const char* sourceFile, unsigned int long line) {
    std::stringstream ss;
...
void
Helpers::logCrashReason(int sig, bool stackTraceIfAvailable, Level level, const char* logger) {
...
Logger*
Loggers::getLogger(const std::string& identity, bool registerIfNotAvailable) {
    return ELPP->registeredLoggers()->get(identity, registerIfNotAvailable);
}

void
Loggers::setDefaultLogBuilder(el::LogBuilderPtr& logBuilderPtr) {
    ELPP->registeredLoggers()->setDefaultLogBuilder(logBuilderPtr);
}

bool
Loggers::unregisterLogger(const std::string& identity) {
    return ELPP->registeredLoggers()->remove(identity);
}

bool
Loggers::hasLogger(const std::string& identity) {
    return ELPP->registeredLoggers()->has(identity);
}

Logger*
Loggers::reconfigureLogger(Logger* logger, const Configurations& configurations) {
    if (!logger)
        return nullptr;
...
Logger*
Loggers::reconfigureLogger(const std::string& identity, const Configurations& configurations) {
    return Loggers::reconfigureLogger(Loggers::getLogger(identity), configurations);
}

Logger*
Loggers::reconfigureLogger(const std::string& identity, ConfigurationType configurationType, const std::string& value) {
...
void
Loggers::reconfigureAllLoggers(const Configurations& configurations) {
...
void
Loggers::reconfigureAllLoggers(Level level, ConfigurationType configurationType, const std::string& value) {
    for (base::RegisteredLoggers::iterator it = ELPP->registeredLoggers()->begin();
         it != ELPP->registeredLoggers()->end(); ++it) {
...
Loggers::defaultConfigurations(void) {
    return ELPP->registeredLoggers()->defaultConfigurations();
}

const base::LogStreamsReferenceMap*
Loggers::logStreamsReference(void) {
    return ELPP->registeredLoggers()->logStreamsReference();
}

base::TypedConfigurations
Loggers::defaultTypedConfigurations(void) {
...
void
Loggers::configureFromGlobal(const char* globalConfigurationFilePath) {
...
void
Loggers::flushAll(void) {
    ELPP->registeredLoggers()->flushAll();
}

void
Loggers::setVerboseLevel(base::type::VerboseLevel level) {
    ELPP->vRegistry()->setLevel(level);
}

base::type::VerboseLevel
Loggers::verboseLevel(void) {
    return ELPP->vRegistry()->level();
}

void
Loggers::setVModules(const char* modules) {
